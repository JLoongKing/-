...
{  
    ...
    function add(){
        ...
        if($tablename!='t_comment'){
            $logdata = ['adminname' => 'Admin', 'loginfo' => '向'.$tablename.'添加'.implode(" ",$data)];
            Db::table('t_log')->insert($logdata);
        }
    ...
    function delete(){
        ...
        $logdata = ['adminname' => 'Admin', 'loginfo' => '从'.$tablename.'删除字段名为'.$fieldname.'值为'.$fieldvalue.'的数据'];
        Db::table('t_log')->insert($logdata);
        return json($model);
    }
    ...
    function update(){
        ...
        $str='从'.$tablename.'修改字段名为'.$fieldname.'值为'.$fieldvalue.'的数据，修改为'.implode(" ",$data);
        $logdata = ['adminname' => 'Admin', 'loginfo' => $str];
        Db::table('t_log')->insert($logdata);
    ...
    function alert(){
        ...
        $logdata = ['adminname' => 'Admin', 'loginfo' => '从'.$tablename.'修改条件为'.implode(" ",$field).'的数据，修改为'.implode(" ",$data)];
        Db::table('t_log')->insert($logdata);
        return json(["code"=>"400","info"=>"修改成功！"]);
    }
    ...
    function upfile(){
        ...
        $logdata = ['adminname' => 'Admin', 'loginfo' => '上传了文件'.$images_name];
        Db::table('t_log')->insert($logdata);
      ...
      public function addclick(Request $request)
      {
          ...
          Db::table('t_blog')->where('id', $id)->setInc('clickHit');
     ...
     //自定义查找方法,$tablename:表名，$sql:查询条件,limit：分页条件,$colname:列名
     function selectcol(){
        $tablename= Request::instance()->post('tablename');
        $colname= Request::instance()->post('colname');
        $sql= Request::instance()->post('sql');
        //将条件由{"id":">,72"}改变为["id"=>">,72"]数组格式
        $sql=json_decode($sql,true); 
        ...
        //无条件的分页查询
        if($sql==null){
            $model=Db::table($tablename)->limit($limit)->field($colname)->select();
            return json($model);
        }
        //有条件的不分页查询
        if($limit==''){
            ...
            //进行查询
            $model=Db::table($tablename)->where($sql)->field($colname)->select();
            return json($model);
        }
        //有条件的分页查询
        //获取所有的key值，value会是这种格式{"id":">,72"}，
        ...
        //进行查询
        $model=Db::table($tablename)->where($sql)->limit($limit)->field($colname)->select();
        return json($model);
    }
    //自定义查找方法,$tablename:表名，$sql:查询条件,limit：分页条件
    function selectcolorder(){
        $tablename= Request::instance()->post('tablename');
        $order= Request::instance()->post('order');
        $colname= Request::instance()->post('colname');
        $sql= Request::instance()->post('sql');
        ...
        //进行查询
        $model=Db::table($tablename)->where($sql)->order($order)->limit($limit)->field($colname)->select();
        return json($model);
    }
} 
